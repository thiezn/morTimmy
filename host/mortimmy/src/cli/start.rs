...
impl StartCommand {
    pub fn merge_config(self, mut config: AppConfig) -> AppConfig {
        ...
        if let Some(health_check_interval_ms) = self.health_check_interval_ms {
            config.session.health_check_interval_ms = health_check_interval_ms;
        }
        if let Some(reconnect_interval_ms) = self.reconnect_interval_ms {
            config.session.reconnect_interval_ms = reconnect_interval_ms;
        }
