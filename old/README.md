...
- 2x L298N Stepper Motor Driver Controller Board WB291111
...
- 3A-6S UBEC for Raspberry Pi 'brand hobbywing'
...
The last bit on my hardware kit list is a camera. I would like to use openCV on the Raspberry Pi so decided to purchase the official Raspberry Pi Camera module and the most recent Raspberry Pi model 2B. This has a quad core processor which will allow me to use multiple threads on the different cores hopefully increasing performance dramatically over the previous Raspberry Pi models.
- Raspberry Pi 2B
