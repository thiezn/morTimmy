...
- The Raspberry Pi host owns networking, routing, telemetry fanout, capture tooling, and optional camera integration.
- Shared crates carry the protocol, units, limits, and driver abstractions so both sides evolve against the same contract.

The codebase is organized as an implementation scaffold, and the package boundaries align with the active hardware direction: Pimoroni Pico LiPo 2, Pico Audio Pack, and Trellis M4 4x4.

The active control design is centered on a desired-state control plane:
...
- one-shot actions such as controller status requests, parameter updates, audio chunks, and Trellis LED updates stay outside that continuous-control path
...
├── integration_test/     # protocol and live-hardware integration harness
└── docs/
    └── src/
```
...
- `FrameDecoder` provides stream-oriented resynchronization at the byte level for host tools and serial/USB bridges.

Message families include:

- `messages::host` for latest-wins control and correlated request envelopes from the brain
- `messages::commands` for desired-state, parameter, audio, and Trellis payloads reused inside those envelopes
- `messages::controller` for response, report, and event envelopes emitted by the controller
- `messages::telemetry` for controller status, control-applied state, range, battery, audio status, and Trellis event payload types
- `WireMessage` as the top-level direction tag shared by both sides of the link
...
The crate centers around a `FirmwareScaffold` that aggregates:
...
The embedded control plane has a single latest-wins apply path for continuous motion:
...
The embedded board profile encodes the repository's active target assumptions:
...
`stationary teleop` is the normal stopped state, while `fault` is reserved for safety failures and explicit fault requests.
...
- `mortimmy-tools deploy firmware` covers the full bring-up loop: build ELF, print artifacts, convert to UF2, deploy through `picotool` when the RP2350 BOOTSEL interface is visible, fall back to a BOOTSEL-mounted Pico volume when it is not, flash through the native `probe-rs` library, and hand off defmt/RTT monitoring to `probe-rs run`.
- The boot path is intentionally safe for bench testing without peripherals: it logs the board profile and default task state, then waits in a safe zero-drive `teleop` state for desired-state traffic.
...
- `brain::autonomy` provides a safe hardcoded autonomous runner that already uses desired-state sequencing
...
The `loopback` transport backend reuses the same host framing code as the live `serial` path, but exchanges bytes with `FirmwareScaffold` in-process so protocol work can be exercised when hardware is not attached or not in BOOTSEL mode.

The host transport contract separates three responsibilities on the same USB CDC link:

- `send_control` for latest-wins control snapshots
- `send_request` for correlated one-shot request/response traffic
- `drain_messages` for controller-originated reports and events that can arrive independently of requests

## Desired-State Brain Model

The host brain owns a single desired snapshot:

- desired mode
- desired drive intent
- desired servo target

Keyboard input mutates that desired snapshot in teleop mode. Autonomous mode mutates it through a small step runner with explicit step conditions. The runner ships with a safe default servo-scan plan so autonomous mode exercises sequencing without commanding blind drive motion.
...
- runtime robot behavior is modeled as a runtime state machine because the mode can change based on user input, reconnects, telemetry, and injected host commands
...
sequenceDiagram
  ...
  Brain->>FW: HostMessage::Control(generation, desired_state)
  FW-->>Brain: ControllerMessage::Report(ControlAppliedReport)
...
The embedded Rust typestate pattern is important, but it is not applied uniformly.
...
`host/tools` is shaped around the same shared protocol rather than placeholder logging only.

- `inspect` parses a recorded capture and reports frame and message counts using the real framing decoder
- `replay` reuses the same capture parsing path and serves as the live replay bridge seam for hardware testing
...
1. A local operator, autonomous runner, or websocket client updates the host-owned desired state.
...
5. Controller status responses, control-applied reports, sensor reports, and controller events flow back through the same shared protocol to the host.
6. The host fans telemetry out to WebSocket clients and tooling or replay pipelines.
...
- RP2350 firmware can be exercised either through a BOOTSEL UF2 upload, preferably via `picotool` on macOS, or through probe-rs with chip `RP235x` when a debug probe is attached.
- The host `start` command supports both loopback and live USB CDC serial transports.
- The RP2350 firmware includes an Embassy USB CDC executor task that applies framed host messages through `FirmwareScaffold` and emits controller messages.
- Continuous control flows through `HostMessage::Control`, and the host runs a built-in autonomous servo-scan plan when autonomous mode is selected.
